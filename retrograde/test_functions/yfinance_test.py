...
def portfolio_chart_data(ticker_symbol, width, date):
    # Define the ticker symbol of the stock you want data for
    if not is_valid_ticker(ticker_symbol):
        return {'ticker': ticker_symbol + ": Not valid Ticker", 'chart_data': []}
    ...
    # Get historical data
    historical_data = ticker.history(start=date-timedelta(days=90), end=date + timedelta(days=1), interval=interval)  # You can specify the period ("1y", "5d", "max", etc.)
    #print("Company Name:", ticker.info['longName'])
    # Print the historical data
    """ for record in ticker.info:
        print(record, ": ", ticker.info[record]) """
        
    #historical_data.to_csv("historical_data " + ticker_symbol + ".csv")
